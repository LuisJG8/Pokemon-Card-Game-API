...
# dictionary to match pokemon element, F -> Fire, WW -> Water, Water, C -> Common, etc
pokemon_types = {
    'R': 'Fire',
    'W': 'Water',
    'G': 'Grass',
...
def pokemon_card_data(pokemon_or_trainer):
    pokemon_info = []
    ...
    if pokemon_or_trainer == 'Pokémon' or pokemon_or_trainer == 'Stage 1' or pokemon_or_trainer == 'Stage 2':
    ...
    if pokemon_or_trainer == 'Pokémon' or pokemon_or_trainer == 'Stage 1' or pokemon_or_trainer == 'Stage 2':
        try:
            ...
            if 'Basic' not in p_card_rarity:
                dot_marker = p_card_rarity.find('.')
                pokemon_stage = p_card_rarity[0:dot_marker]
                pokemon_pre_evolution = p_card_rarity[dot_marker+1:]
                ...
                # pokemon_or_trainer = cleaned_rarity[0].strip()    
                
                print('yeeeeeeeeee', p_card_type, pokemon_stage.strip(), pokemon_pre_evolution.strip())
                pokemon_info.extend([p_card_type, pokemon_stage.strip(), pokemon_pre_evolution.strip()])
    ...
    elif pokemon_or_trainer == 'Trainer':
        card_rarity = driver.find_element(By.CLASS_NAME, value="card-text-type")
        cleaned_rarity = clean_data_hyphen(card_rarity.text.replace(' - Evolves from ', ' . '))
        p_card_type = cleaned_rarity[1].strip()
        pokemon_info.extend([pokemon_or_trainer + '/' + p_card_type, ''])
    ...
    if pokemon_or_trainer == 'Pokémon' or pokemon_or_trainer == 'Stage 1' or pokemon_or_trainer == 'Stage 2':
        try:
            ...
            try:
                ...
                if second_attack:
                    element_list_for_attack_one = []
                    element_list_for_attack_two = []

                    for y in (card_attack_energy.text):
                        print(pokemon_types.get(y, 'N/A'))
                        hey = pokemon_types.get(y, 'N/A')
                        element_list_for_attack_one.append(hey)
                    print(element_list_for_attack_one)

                    for z in (second_attack_types):
                        print(pokemon_types.get(z, 'N/A'))
                        hey = pokemon_types.get(z, 'N/A')
                        element_list_for_attack_two.append(hey)
                    print(element_list_for_attack_two)

                    list_of_attack_s_data = [
                                                {
                                                'name'        : card_attack_name_clean,
                                                'damage'      : card_attack_points_clean,
                                                'energy'      : element_list_for_attack_one,
                                                **({'attack description': card_attack_description.text.replace("", '')} if card_attack_description.text.replace("", '') else {})
                                            },
                                            {
                                                'name'         : second_card_attack_name_clean,
                                                'damage'       : second_card_attack_points_clean,
                                                'energy'       : element_list_for_attack_two,
                                                'description'  : card_attack_description_two.text.replace("", '')
                                            }
                                            ]
                ...
                only_one_attack_list = [{
                                        ...
                                        'damage': card_attack_points_clean,
                                        'energy': element_list,
                                        **({'description': card_attack_description.text.replace("", '')} if card_attack_description.text.replace("", '') else {})
                                    }]
    ...
    if pokemon_or_trainer == 'Pokémon' or pokemon_or_trainer == 'Stage 1' or pokemon_or_trainer == 'Stage 2':
        try:
            ...
            print('no weaknes and no retreat')
            pokemon_info.extend(['', ''])
    else:
        pokemon_info.extend(['', ''])
    ...
    try:
        ...
        if pokemon_pack:
           start_paren = pokemon_pack.text.find('(')
           end_paren   = pokemon_pack.text.find(')')

           print(pokemon_pack.text[start_paren+1:end_paren])
        
        card_id_pack = pokemon_pack.text[start_paren+1:end_paren]
...
if __name__ == '__main__':
    ...
    for card_index in range(the_card):
        ...
        cleaned_rarity = clean_data_hyphen(card_type.text)
        pokemon_or_trainer = cleaned_rarity[0].strip()
        print('THIS IS THE P CARD TYPE: ', pokemon_or_trainer)

        # main function calls
        pokemon_card_data(pokemon_or_trainer=pokemon_or_trainer)
